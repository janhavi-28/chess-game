...
async function main() {
  ...
  try {
    ...
    const fen = data.fen;
    const moveEvents = data.moves;
    const uciMoves = moveEvents.map(m => m.uci);
    ...
    fs.writeFileSync(movesFilePath, JSON.stringify({ 
      moveList: uciMoves, 
    ...
    try {
      execSync(`start explorer "${path.resolve(__dirname, 'out')}"`);
    } catch (e) {
